...
class TTSRequest(BaseModel):
    ...
    emo_audio_prompt: Optional[str] = Field(None, description="情绪参考音频")
    emo_alpha: float = Field(1.0, description="情绪强度/混合系数")
    emo_vector: Optional[List[float]] = Field(None, description="情绪维度向量")
    use_emo_text: bool = Field(False, description="是否使用根据text推断的情绪")
    emo_text: Optional[str] = Field(None, description="用于推断情绪向量的文本")
    use_random: bool = Field(False, description="是否启用随机情绪向量抽取")
